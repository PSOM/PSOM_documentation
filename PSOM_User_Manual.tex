...
\end{lstlisting}
    ...
    \item Within \text{src}, type the following commands:
...
% \centering 
%   \includegraphics[width=0.5\textwidth]{grid_z.pdf}
...
The parameter \Blue{D} is the z-coordinate (negative value) of the topography as a function of $x$ and $y$; \Blue{dzbot} and \Blue{dztop} are the thicknesses (positive values) for the bottom and top layers, respectively. These three parameters are defined independently from the vertical stretching.\par%, but grid sizes in the whole domain have to be varying smoothly in all directions.\par
